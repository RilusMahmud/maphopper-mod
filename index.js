...
proxyService.use(
  "/navigate/directions/v5/gh/driving-traffic",
  proxy(GH_BASE, {
    proxyReqPathResolver(req) {
      let url = req.url.replace("/", "/route");
      logProxyMessage(url);
      return url;
    },
    userResDecorator: function (proxyRes, proxyResData, userReq, userRes) {
      ...
      if (userRes.statusCode !== 200) {
        ...
        var mapBoxResponse = directionsMapper.getMapping(
          data,
          userReq.query.profile,
          userReq.query.locale,
          userReq.query.mapboxkey,
          userReq.query.geometries
        );
        log.info(SUCC_MSG);
  ...
  proxy(GH_BASE, {
    ...
    userResDecorator: function (proxyRes, proxyResData, userReq, userRes) {
      ...
      if (userRes.statusCode !== 200) {
        ...
        let res = isoMapper.getMapping(
          data,
          userReq.query.time_limit,
          userReq.query.buckets,
          userReq.query.contours_colors
        );
        log.info(SUCC_MSG);
  ...
  proxy(GH_BASE, {
    ...
    userResDecorator: function (proxyRes, proxyResData, userReq, userRes) {
      ...
      if (userRes.statusCode !== 200) {
        ...
        let res = matrixMapper.getMapping(
          data,
          userReq.query.out_array,
          userReq.query.from_point,
          userReq.query.to_point,
          userReq.query.point
        );
        log.info(SUCC_MSG);
        return JSON.stringify(res);
